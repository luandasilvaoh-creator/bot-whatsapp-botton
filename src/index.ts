...
//   } else {
//     res.status(404).send('Arquivo dashboard.html não encontrado na raiz do projeto');
//   }
// });
...
//     'Veja os detalhes nos tópicos da lista abaixo 👇'
//   );
// }

// function saveLead(remoteJid: string, name: string, option?: string, deviceType?: string) {
//   try {
//     const number = remoteJid.replace(/@s\.whatsapp\.net$/, '');

//     if (remoteJid.includes('@g.us') || number.includes('@g.us')) return;

//     const leadDir = path.join(process.cwd(), 'src', 'leads');

//     if (!fs.existsSync(leadDir)) {
//       fs.mkdirSync(leadDir, { recursive: true });
//     }

//     const filePath = path.join(leadDir, `${number}.json`);
//     const now = new Date();

//     const resolvedDeviceType = deviceType || leadStates[remoteJid]?.deviceType || 'UNKNOWN';
...
//       lastMessageFormatted: `${now.toLocaleDateString('pt-BR')} ${now.toLocaleTimeString('pt-BR', {
//         hour: '2-digit',
//         minute: '2-digit',
//       })}`,
...
//         lastOption: option || existing.lastOption,
//         deviceType: resolvedDeviceType || existing.deviceType || 'UNKNOWN',
//       };
//     }

//     fs.writeFileSync(filePath, JSON.stringify(leadData, null, 2));
//     logInfo(`Lead atualizado: ${leadData.number} (${leadData.name}) • 📱 Device: ${leadData.deviceType}`);
//   } catch (err) {
//     logError('Falha ao salvar lead', err);
//   }
// }

// async function sendMenuButtons(remoteJid: string) {
...
// ------------------------- */
// async function sendAdaptiveResourcesMessage(remoteJid: string, deviceType: string) {
//   const isIPhone = deviceType === 'IPHONE';
  
//   if (isIPhone) {
//     // 📱 iPhone: Usar lista simples (mais compatível)
//     logInfo(`📱 iPhone detectado: enviando lista simples para ${remoteJid}`);
...
//               rowId: 'RECURSO_MULTICANAL',
//               description: 'WhatsApp, Instagram, Facebook em um só lugar'
//             },
//             { 
//               title: '🤖 Automações & Chatbots', 
//               rowId: 'RECURSO_AUTOMACAO',
//               description: 'Atendimento 24h inteligente'
//             },
//             { 
//               title: '📊 Relatórios & Controle', 
//               rowId: 'RECURSO_RELATORIOS',
//               description: 'Métricas e performance da equipe'
//             },
//             { 
...
//   } else {
//     // 🤖 Android/Web: Usar carousel (experiência premium)
//     logInfo(`🤖 Android/Web detectado: enviando carousel para ${remoteJid}`);
    
//     try {
//       const imageMessage = await prepareLocalImage(sock!, 'simular.png');
...
//           },
//           body:
//             'Métricas de atendimento, performance da equipe\n' +
//             'e histórico completo de conversas.',
//           footer: 'MultSolutions',
//           buttons: [
//             {
...
//       await sendCarousel(sock!, remoteJid, carouselCards);
      
//     } catch (error) {
//       logError('Erro ao enviar carousel, fallback para lista', error);
      
//       // Fallback: se carousel falhar, enviar lista
//       await sock!.sendMessage(remoteJid, {
//         text: textoRecursosResumo(),
//         buttonText: 'Ver Recursos',
//         sections: [
...
//             title: 'Recursos Disponíveis',
//             rows: [
//               { title: '📨 Multicanal', rowId: 'RECURSO_MULTICANAL' },
//               { title: '🤖 Automações', rowId: 'RECURSO_AUTOMACAO' },
//               { title: '📊 Relatórios', rowId: 'RECURSO_RELATORIOS' },
...
// /* -------------------------
//    🔥 EXTRATOR DE COMANDO COMPLETO (CORRIGIDO)
// ------------------------- */
...
//   // 🔥 BOTÃO NOVO (interactiveResponseMessage)
...
//   // 🔥 CAROUSEL RESPONSE (nativeFlowResponseMessage direto)
...
//   // 🔥 LIST RESPONSE v2 (algumas variações)
...
// /* -------------------------
//    🔍 DEBUG DE MENSAGENS (NOVO)
//    Ajuda a identificar formatos não capturados
// ------------------------- */
// function debugMessage(msg: proto.IWebMessageInfo) {
//   const types = [];
  
//   if (msg.message?.conversation) types.push('conversation');
//   if (msg.message?.extendedTextMessage) types.push('extendedText');
//   if ((msg.message as any)?.listResponseMessage) types.push('listResponse');
//   if ((msg.message as any)?.buttonsResponseMessage) types.push('buttonResponse');
//   if ((msg.message as any)?.templateButtonReplyMessage) types.push('templateButton');
//   if ((msg.message as any)?.interactiveResponseMessage) types.push('interactive');
//   if ((msg.message as any)?.nativeFlowResponseMessage) types.push('nativeFlow');
  
//   if (types.length > 0) {
//     logInfo(`📱 Tipos de mensagem detectados: [${types.join(', ')}]`);
//   } else {
//     logWarn('⚠️ Formato de mensagem não reconhecido - veja estrutura completa no console');
...
//   // 📱 iPhone (listas e respostas nativas)
//   if (
...
//     // Debug da mensagem (ajuda a identificar problemas)
//     debugMessage(msg);
...
//     // 🔥 DETECÇÃO MELHORADA DE DISPOSITIVO
...
//       await sendMenuButtons(remoteJid);
...
//     // 🔥 TRATAMENTO DE RESPOSTAS DOS RECURSOS (LISTA DO IPHONE)
...
//         // Reenviar menu para continuar navegando
//         setTimeout(async () => {
//           await sock!.sendMessage(remoteJid, {
...
//         leadStates[remoteJid].tempData = {};
//         await sock!.sendMessage(remoteJid, {
//           text: '📅 Vamos agendar sua demonstração!\n\nPor favor, me informe seu *nome completo*:',
//         });
//         break;
...
//         // 🔥 USA O SISTEMA ADAPTATIVO
//         await sendAdaptiveResourcesMessage(
//           remoteJid, 
//           leadStates[remoteJid].deviceType || 'ANDROID'
//         );
...
//         await sock!.sendMessage(remoteJid, {
//           text: '🎁 Ative seu *teste grátis de 30 dias* agora!\n👉 https://multsolutions.com.br/teste-gratis',
...
//       case 'TESTAR_SISTEMA':
//         logChoice(remoteJid.replace(/@s\.whatsapp\.net$/, ''), 'TESTAR_SISTEMA');
//         await sock!.sendMessage(remoteJid, {
//           image: { url: path.join(__dirname, '../simular.png') },
//           caption:
//             '🚀 *Teste o sistema MultSolutions agora mesmo!*\n\nClique abaixo para acessar o ambiente de teste gratuito 👇\n\n🔗 https://leo.multsolutions.com.br/teste-gratis',
//           footer: 'MultSolutions Automação',
//           buttons: [{ buttonId: 'MENU', buttonText: { displayText: '🔙 Voltar ao Menu' }, type: 1 }],
//           headerType: 4,
//         } as any);

//         const numberTeste = remoteJid.replace(/@s\.whatsapp\.net$/, '');
//         const filePathTeste = path.join(__dirname, 'leads', `${numberTeste}.json`);
//         let leadInfoTeste: any = {};
//         if (fs.existsSync(filePathTeste)) {
//           leadInfoTeste = JSON.parse(fs.readFileSync(filePathTeste, 'utf-8'));
//         }

//         const mensagemAtendenteTeste =
//           `🧪 *Novo Lead testando o sistema:*\n\n` +
//           `👤 Nome: ${leadInfoTeste.name || 'Não informado'}\n` +
//           `📱 Número: ${leadInfoTeste.number}\n` +
//           `📱 Dispositivo: ${leadInfoTeste.deviceType || 'UNKNOWN'}\n` +
//           `🕒 Última interação: ${leadInfoTeste.lastMessageFormatted || 'N/A'}\n` +
//           `💬 Última opção: ${leadInfoTeste.lastOption || 'N/A'}\n` +
//           `🔁 Interações: ${leadInfoTeste.interactionCount || 0}\n\n` +
//           `🧭 Origem: ${leadInfoTeste.source || 'WhatsApp'}\n` +
//           `📅 Entrou em: ${leadInfoTeste.joinedAt ? new Date(leadInfoTeste.joinedAt).toLocaleString('pt-BR') : 'N/A'}`;

//         await sock!.sendMessage('558584460424@s.whatsapp.net', { text: mensagemAtendenteTeste });
//         saveLead(remoteJid, pushName, 'TESTAR_SISTEMA', leadStates[remoteJid].deviceType);
//         break;

//       case 'FALAR_ATENDENTE':
//         logChoice(remoteJid.replace(/@s\.whatsapp\.net$/, ''), 'FALAR_ATENDENTE');
//         await sock!.sendMessage(remoteJid, {
//           text: '👨‍💼 Um atendente entrará em contato em instantes. Aguarde um momento.',
//           footer: 'MultSolutions Automação',
//           buttons: [{ buttonId: 'MENU', buttonText: { displayText: '🔙 Voltar ao menu' }, type: 1 }],
//           headerType: 1,
//         } as any);
...
//         const number = remoteJid.replace(/@s\.whatsapp\.net$/, '');
//         const filePath = path.join(__dirname, 'leads', `${number}.json`);
//         let leadInfo: any = {};
//         if (fs.existsSync(filePath)) {
...
//           `📅 Entrou em: ${leadInfo.joinedAt ? new Date(leadInfo.joinedAt).toLocaleString('pt-BR') : 'N/A'}`;

//         await sock!.sendMessage('558584460424@s.whatsapp.net', { text: mensagemAtendente });
//         saveLead(remoteJid, pushName, 'FALAR_ATENDENTE', leadStates[remoteJid].deviceType);
//         break;
...
//           logInfo('✅ QR Code gerado! Acesse http://localhost:8000');
//         } catch (err) {
...
//     // 🔥 REGISTRA NO HISTÓRICO
//     logBotMessage(remoteJid, pushName, '✨ Enviou carousel com recursos da plataforma [3 cards]');
    
//     logInfo(`✅ Carousel enviado com sucesso para ${remoteJid}`);
    
//   } catch (error) {
//     logError('❌ Erro ao enviar carousel, usando fallback', error);
